...
stock FindDynamicObject(playerid, Float:findradius, Float:streamdistance = 20.0){
	new buffer[256], szLIST[768], cnt = 0, Float:px, Float:py, Float:pz, areaid, priority, index,
...
CMD:saoicapacity(playerid){
	if(!IsPlayerAdmin(playerid)) return 0;
	new buffer[256];
	format(buffer,sizeof buffer,"{00AAFF}SAOI File loaded: {00FF00}%d/%d",CountSAOIFileLoaded(),_:MAX_SAOI_FILE);
	SendClientMessage(playerid,0xFFFFFFFF,buffer);
	return 1;
}
...
CMD:saoiinfo(playerid,params[]){
	...
	ShowPlayerDialog(playerid,DIALOG_NONE,DIALOG_STYLE_MSGBOX,"Saoi File Information", szLIST, "Exit", "");
	return 1;
}
